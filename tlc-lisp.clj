...
(defn controlar-aridad [lis val-esperado] 
	(if (= (count lis) val-esperado ) val-esperado 
...
(defn igual? [a b]
	(cond
		(= a b) true
		(= (.toUpperCase (str a)) (.toUpperCase (str b))) true
		(and (nil? a) (seq? b) (empty? b)) (and (nil? b) (seq? a) (empty? a)) true
		(and (nil? a) (nil? b)) (and (seq? a) (empty? a) (seq? b) (empty? b)) true
...
(defn imprimir 
    ([elem]
        (cond 
		(and (not (seq? elem)) (= elem \space)) (do
							(flush)
							elem
							)
		...
		(= '*error* (first elem)) (imprimir elem elem) 
...
(defn actualizar-amb [amb-global clave valor]
  (let [result (.indexOf amb-global clave)]
     	(if (neg? result) (concat amb-global (list clave valor))
                        (seq (assoc (into [] amb-global) (+ (.indexOf amb-global clave) 1) valor)) 
		)
  )
)
...
(defn revisar-f [lis] 
	(if (and (list? lis) (= (first lis) '*error*)) lis nil)
)
...
(defn revisar-lae [lis]
	...
	(def descartar_elementos (fn [lis] 
			(if (not (list? (first lis))) (descartar_elementos (next lis)) (first lis)) 
		)
	)
	(cond
		(not (sequential? lis)) nil 
		(= (first lis) '*error*) lis
		
		(and (not(= (first lis) '*error*)) (not(some list? lis))) nil	
																		
		
		(and (not(= (first lis) '*error*)) (some list? lis)) 
			(if (nil? (revisar-lae(descartar_elementos lis)))
				(revisar-lae (next lis)) 					 
				(revisar-lae (descartar_elementos lis)) ) 	

		(not (list? (first lis))) 	  
			(if (not (nil? lis)) (revisar-lae (next lis))) 
		(and (not (= (first (first lis)) '*error*)) (not(some list? (first lis)))) 
															
			(if (not (nil? lis)) (revisar-lae (next lis)))  
			
		(list? (first lis)) (revisar-lae (first lis)) 
...
(defn buscar [elem lis]
	(if (or (empty? lis) (odd? (count lis))) (list '*error* 'unbound-symbol elem) (if (= elem (first lis)) (first (rest lis)) (buscar elem (rest(rest lis)))))
)
...
(defn evaluar-cond [lis amb-global amb-local]
	(cond
		...
		(not  (igual? (first (evaluar (first(first lis)) amb-global amb-local)) nil )) (evaluar-secuencia-en-cond (next(first lis)) amb-global amb-local)
